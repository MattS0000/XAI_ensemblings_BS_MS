...
def _overlap(tensor1: torch.Tensor, tensor2: torch.Tensor, threshold1: float = 0.0, threshold2: float = 0.0) -> torch.Tensor:
    return torch.logical_and(torch.abs(tensor1) > threshold1, torch.abs(tensor2) > threshold2)


def _union(tensor1: torch.Tensor, tensor2: torch.Tensor, threshold1: float = 0.0, threshold2: float = 0.0) -> torch.Tensor:
    return torch.logical_or(torch.abs(tensor1) > threshold1, torch.abs(tensor2) > threshold2)
...
def consistency(explanations: list[torch.Tensor]) -> torch.Tensor:
    ...
    """
    diffs = [_matrix_norm_2(exp1, exp2) for exp1, exp2 in itertools.combinations(explanations, 2)]
    return 1 / max(diffs) + 1


def stability(explanator: Attribution, model, image: torch.Tensor, epsilon: float = 0.1) -> torch.Tensor:
    """
...
def decision_impact_ratio(dataset: Dataset, predictor: Callable[..., torch.Tensor], explanator: Attribution,
...
def confidence_impact_ratio(dataset: Dataset, probs: Callable[..., torch.Tensor], explanator: Attribution,
                            baseline: torch.Tensor) -> torch.Tensor:
    """
...
def accordance_recall(mask: torch.Tensor, explanation: torch.Tensor, threshold: float = 0.5) -> torch.Tensor:
    """
    Opis: Mierzy jaką część maski wykryło wyjaśnienie.
    Argumenty wejściowe: torch.Tensor (maska obrazu), torch.Tensor (wyjaśnienie/obszar krytyczny), torch.Tensor/skalar (opcjonalnie,
...
def accordance_precision(mask: torch.Tensor, explanation: torch.Tensor, threshold: float = 0.5) -> torch.Tensor:
    """
    Opis: mierzy jaką część wyjaśnienia stanowiła maska.
    Argumenty wejściowe: torch.Tensor (maska obrazu), torch.Tensor (wyjasnienie/obszar krytyczny), torch.Tensor/skalar (opcjonalnie,
    jaka minimalna wartość w wyjaśnieniu ma być uznana za ważną)
    Wartości wyjściowe: torch.Tensor (wynik metryki)
    :return:
    Let S(𝑥) be the suspicious pneumonia area that is annotated
...
def set_accordance_recall(masks, explanations, threshold = 0.5):
    ...
    recalls = [accordance_recall(mask, expl, threshold=threshold).item() for mask, expl in zip(masks, explanations)]
    return recalls
...
def set_accordance_precision(masks, explanations, threshold = 0.5):
    ...
    precs = [accordance_precision(mask, expl, threshold=threshold).item() for mask, expl in zip(masks, explanations)]
    return precs


def F1_score(masks: list[torch.Tensor], explanations: list[torch.Tensor], threshold: float = 0.5) -> torch.Tensor:
    """
...
is identified by the interpretation method
    ...
    values = [2*(recall*prec)/(recall+prec) for (recall, prec) in zip(acc_recall, acc_prec)]
    return sum(values)/len(values)


def intersection_over_union(masks: list[torch.Tensor], explanations: list[torch.Tensor], threshold: float = 0.5) -> torch.Tensor:
    """
...
is identified by the interpretation method
    ...
    """
    values = [torch.sum(_overlap(expl, mask, threshold1=threshold))/torch.sum(_union(expl, mask, threshold1=threshold)) for (expl, mask) in zip(explanations, masks)]
    return sum(values)/len(values)


def ensemble_score(weights: Union[list, torch.Tensor],
                   metrics_scores: Union[list[torch.Tensor], torch.Tensor]) -> torch.Tensor:
    """
    Opis: średnia ważona innych metryk. Ensemble_score(wagi, metryki) -> torch.tensor
    Argumenty wejściowe: torch.Tensor/Lista (lista z wagami dla poszczególnych metryk), torch.Tensor/Lista (lista z wynikami
    poszczególnych metryk)
    Wartości wyjściowe: torch.Tensor (wynik metryki)
    :return:
    ES(w, M) = sum_{i}(w_i * M_i)
    """
    return sum([weight*metric_score for (weight, metric_score) in zip(weights, metrics_scores)])
